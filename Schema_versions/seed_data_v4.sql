-- Stock Daddy: seed data for schema_v4.sql
-- Run schema_v4.sql first.
--
-- Unchanged from seed_data_v3.sql other than this header: v4 only fixed a
-- view definition and added CHECK constraints, neither of which required
-- new data. All values here already satisfy v4's new constraints
-- (replacement costs, dispatch costs, distances, and rates are all
-- positive), and every building/item-type combination already has a
-- target_quantity row, so this instance does not exercise v4's view fix -
-- see schema_v4.sql's changelog for a case that would.
--
