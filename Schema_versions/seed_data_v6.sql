-- Stock Daddy: seed data for schema_v6.sql (SQLite)
-- Run schema_v6.sql first (or just run build_db.py, which does both).
--
-- Same five-site roofing scenario as seed_data_v5.sql, plus:
--   - a service_life_days value on one item type (Nail Gun), demonstrating
--     the REQ-23 fix
--   - one status-only item_movement row (no location change), demonstrating
--     the REQ-11 fix - see the bottom of this file
--
...
--
-- service_life_days (REQ-23): set for the two types where "how old is too
-- old" actually matters - nail guns wear out mechanically, hard hats have a
-- real safety-driven replacement interval. Left NULL for the rest, which
-- exercises the "no limit configured" case, not just the "has a limit" case.
-- --------------------------------------------------------------------
INSERT INTO item_type (name, description, replacement_cost, service_life_days) VALUES
  ('Nail Gun', 'Pneumatic or electric nail gun', 340.00, 730),   -- item_type_id 1, 2 years
  ('Air Compressor', 'Portable air compressor', 620.00, NULL),   -- item_type_id 2
  ('Extension Ladder', 'Extension or step ladder', 290.00, NULL),-- item_type_id 3
  ('Hard Hat', 'Safety helmet', 38.00, 1825),                    -- item_type_id 4, 5 years
  ('Safety Harness', 'Fall-protection harness', 145.00, NULL),   -- item_type_id 5
  ('Tear-off Shovel', 'Roofing tear-off shovel', 42.00, NULL);   -- item_type_id 6
...
-- REQ-11 fix, demonstrated: Nail Gun #4 went out with a crew two days ago -
-- still sitting in the same Salina North locker (storage 1), so no location
-- change at all, but its status moved from In Storage to In Use. Before v6
-- this event had nowhere to be recorded.
INSERT INTO item_movement (item_id, from_status, to_status, moved_at)
SELECT item_id, 'In Storage', 'In Use', datetime('now', '-2 days')
  FROM item WHERE item_name = 'Nail Gun #4';
